...
## 🔄 وضعیت راند ۱۶ (۲۰۲۶-۰۹-۲۲) — اسپات کریپتوکاوی

| مورد | وضعیت |
|---|---|
| شاخهٔ فعال | `round16-spot-alerts` (پوش‌شده؛ merge با فرمان Viva) |
| موتور اسپات | کاملاً مستقل از فیوچرز — ستاپ `SPOTBREAK`، تریگر `4h/8h/12h/1d/3d`، شناسهٔ `VIVA-SPOT-E######`، سقف استاپ ۱۰٪ |
| لَدر هشدار | 🖐 برخورد اولیه → ⏳ نزدیک شکست → 💥 شکست نزولی (فقط هشدار) → ✅ تأیید فقط با کلوز صعودی معتبر |
| چارت | لگاریتمی (گارد <۳٪)، باکس سبز تا سقف ساختاری از اولین هشدار، بدون ابزار فیوچرزی، لیبل VIVA-SPOT-MON |
| تست‌ها | ۳۴۹ پاس / ۱ اسکیپ (`python -m pytest tests/ -q`) |

📚 **کیت انتقال پروژه (منبع حقیقت برای ادامهٔ کار):** `WORKLOG.md` (وضعیت زنده + صف بعدی + پروتکل پیوستگی) · `handoff/HANDOFF.md` · `handoff/KARTABEL.md` · `handoff/PROJECT_MAP.md` · `handoff/VIVA_LAWS.md`

> ⚠️ `CHAT_CONTEXT.md` منسوخ است — نخوانید (F15؛ با merge بعدی حذف می‌شود).
